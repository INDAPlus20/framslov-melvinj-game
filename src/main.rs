...
struct MainState {
    ...
    script1: Option<String>,
}

impl MainState {
    fn new(ctx: &mut Context, script_name: Option<String>) -> GameResult<MainState> {
...
impl EventHandler for MainState {
    fn update(&mut self, ctx: &mut Context) -> GameResult {
        ...
        while timer::check_update_time(ctx, DESIRED_FPS) {
            ...
            self.input2 = ai_generate_input(&self.gamestate);
            ai_handle_input(&mut self.player2);
...
fn ai_generate_input(gamestate: &mut GameState, name: &str) -> &mut InputState {
    let lib = Library::new(name.replace("rs","dll")).unwrap();
    unsafe {
        let func: Symbol<AIFunc> = lib.get(b"calculate_move").unwrap();
        let input = func(gamestate);
        input
    }
}
...
pub fn main() -> GameResult {
    ...
    for path_prewrap in paths {
        ...
        //Not huge error yet, store the script file name
    ...
    let game = &mut MainState::new(ctx, maybe_name)?;
